...
describe('rest spot stamina regeneration', () => {
  it('stamina regenerates when climbing on a rest spot and not moving', () => {
    ...
    const next = updateStamina(state, {
      isClimbing: true, isSprinting: false, isGrounded: false, isOnRestSpot: true,
    }, 1.0);
  ...
  it('stamina does NOT regenerate when climbing but not on a rest spot', () => {
    ...
    const next = updateStamina(state, {
      isClimbing: true, isSprinting: false, isGrounded: false, isOnRestSpot: false,
    }, 1.0);
  ...
  it('rest spot regen caps at max stamina', () => {
    const state = { current: STAMINA_CONSTANTS.MAX - 1, isDepleted: false, depletedTimer: 0, isClimbing: true };
    const next = updateStamina(state, {
      isClimbing: true, isSprinting: false, isGrounded: false, isOnRestSpot: true,
    }, 10);
