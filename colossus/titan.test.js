...
describe("createTitanDefinition", () => {
  ...
  it("shell_main is the largest part by volume", () => {
    ...
    for (const part of def.parts) {
      ...
      assert.ok(
        shellVol >= partVol,
        `shell_main volume ${shellVol} should be >= ${part.id} volume ${partVol}`,
      );
    }
  });
...
describe("generateTitanSurfacePatches", () => {
  ...
  it("shell_main has the most patches of any single part", () => {
    ...
    for (const [partId, count] of Object.entries(counts)) {
      if (partId !== "shell_main") {
        assert.ok(
          shellMainCount > count,
          `shell_main (${shellMainCount}) should have more patches than ${partId} (${count})`,
        );
      }
    }
  });
  ...
  it("total patch count is reasonable for titan size", () => {
    ...
    assert.ok(
      patches.length > 500,
      `only ${patches.length} patches, expected more for titan-sized body`,
    );
...
describe("getTitanWeakPointPositions", () => {
  ...
  it("positions shift with colossus world position", () => {
    ...
    for (let i = 0; i < at0.length; i++) {
      assert.ok(
        Math.abs(at10[i].x - at0[i].x - 10) < 0.01,
        `part ${at0[i].bodyPartId} x offset wrong`,
      );
    }
  });
});
...
describe("buildCombatWeakPoints", () => {
  ...
  it("head has highest health (healthMultiplier 3.0)", () => {
    ...
    assert.ok(
      head.maxHealth > center.maxHealth,
      `head ${head.maxHealth} should be > center ${center.maxHealth}`,
    );
    assert.ok(
      head.maxHealth > side.maxHealth,
      `head ${head.maxHealth} should be > side ${side.maxHealth}`,
    );
  });
  ...
  it("center rune has higher health than side runes", () => {
    ...
    assert.ok(
      center.maxHealth > left.maxHealth,
      `center ${center.maxHealth} should be > left ${left.maxHealth}`,
    );
    assert.ok(
      center.maxHealth > right.maxHealth,
      `center ${center.maxHealth} should be > right ${right.maxHealth}`,
    );
  });
...
describe("Titan AI Phase 1", () => {
  it("Idle transitions to Patrol after idleDuration", () => {
    const state = createTitanBehaviorState();
    const result = updateTitanBehavior(
      state,
      TITAN_BEHAVIOR_CONFIG,
      TITAN_BEHAVIOR_CONFIG.idleDuration,
      { x: 0, y: 0, z: 0 },
      { x: 0, y: 0, z: 0 },
    );
  ...
  it("Idle does not transition before idleDuration", () => {
    ...
    const result = updateTitanBehavior(
      state,
      TITAN_BEHAVIOR_CONFIG,
      TITAN_BEHAVIOR_CONFIG.idleDuration - 0.1,
      { x: 0, y: 0, z: 0 },
      { x: 0, y: 0, z: 0 },
    );
  ...
  it("Patrol moves at slow patrol speed", () => {
    ...
    const result = updateTitanBehavior(state, TITAN_BEHAVIOR_CONFIG, 1.0, farPlayer, {
      x: 0,
      y: 0,
      z: 0,
    });
    ...
    assert.ok(
      result.position.x <= TITAN_BEHAVIOR_CONFIG.patrolSpeed + 0.01,
      "should not exceed patrol speed",
    );
  });
  ...
  it("Patrol transitions to Aggro when player enters detection range", () => {
    ...
    const result = updateTitanBehavior(state, TITAN_BEHAVIOR_CONFIG, 0.1, playerPos, {
      x: 0,
      y: 0,
      z: 0,
    });
...
describe("Titan AI Phase 2", () => {
  ...
  it("moves faster in aggro during phase 2", () => {
    const state1 = createTitanBehaviorState({
      state: ColossusState.AGGRO,
      phase: 1,
      attackCooldown: 100,
    });
    const state2 = createTitanBehaviorState({
      state: ColossusState.AGGRO,
      phase: 2,
      attackCooldown: 100,
    });
  ...
  it("submerge cycle: warning then active then reset", () => {
    ...
    state = updateTitanBehavior(
      state,
      config,
      config.submergeWarningTime + 0.1,
      playerPos,
      colossusPos,
    );
...
describe("getShellTilt", () => {
  ...
  it("tilt angle is clamped to max", () => {
    ...
    const tilt = getShellTilt(state, TITAN_BEHAVIOR_CONFIG);
    assert.ok(
      tilt.angle <= TITAN_BEHAVIOR_CONFIG.tiltMaxAngle + 0.01,
...
describe("getShockwaveForce", () => {
  ...
  it("force is stronger when closer", () => {
    const state = createTitanBehaviorState();
    const colossusPos = { x: 0, y: 0, z: 0 };
    const forceNear = getShockwaveForce(
      state,
      TITAN_BEHAVIOR_CONFIG,
      { x: 3, y: 0, z: 0 },
      colossusPos,
    );
    const forceFar = getShockwaveForce(
      state,
      TITAN_BEHAVIOR_CONFIG,
      { x: 12, y: 0, z: 0 },
      colossusPos,
...
describe("createTitanMesh", () => {
  ...
  it("children are keyed by part id", () => {
    ...
    for (const part of def.parts) {
      assert.ok(
        mesh.children.some((c) => c.partId === part.id),
  ...
  it("children inherit position from definition", () => {
    ...
    for (const part of def.parts) {
      ...
      assert.ok(
        Math.abs(child.position.x - part.position.x) < 0.01,
        `${part.id} x: ${child.position.x} != ${part.position.x}`,
      );
      assert.ok(
        Math.abs(child.position.y - part.position.y) < 0.01,
        `${part.id} y: ${child.position.y} != ${part.position.y}`,
      );
      assert.ok(
        Math.abs(child.position.z - part.position.z) < 0.01,
  ...
  it("children inherit rotation from definition", () => {
    ...
    for (const part of def.parts) {
      const child = mesh.children.find((c) => c.partId === part.id);
      assert.ok(child !== undefined, `missing ${part.id}`);
      assert.ok(Math.abs(child.rotation.x - part.rotation.x) < 0.01, `${part.id} rot x wrong`);
      assert.ok(Math.abs(child.rotation.y - part.rotation.y) < 0.01, `${part.id} rot y wrong`);
      assert.ok(Math.abs(child.rotation.z - part.rotation.z) < 0.01, `${part.id} rot z wrong`);
    }
  });
  ...
  it("underbelly material is smoother than shell", () => {
    ...
    assert.ok(
      belly.material.roughness < shell.material.roughness,
...
describe("animateTitan", () => {
  ...
  it("applies shell tilt from ai state", () => {
    ...
    assert.ok(shell !== undefined);
    assert.ok(
      Math.abs(shell.rotation.z - 0.2) < 0.01,
  ...
  it("animates legs over time", () => {
    ...
    assert.ok(leg0y !== leg1y || leg0rx !== leg1rx, "legs should animate with time");
  });
