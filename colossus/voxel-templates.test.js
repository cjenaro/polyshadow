...
describe('voxelSphere', () => {
  ...
  it('all voxels are within radius', () => {
    ...
    for (const v of voxels) {
      ...
      assert.ok(dist <= radius + 0.5, `voxel at ${v.x},${v.y},${v.z} dist ${dist} exceeds radius ${radius}`);
    }
  });
...
describe('voxelCylinder', () => {
  ...
  it('all voxels are within the maximum radius', () => {
    ...
    for (const v of voxels) {
      ...
      assert.ok(dist <= maxR + 0.5, `voxel at ${v.x},${v.y},${v.z} dist ${dist} exceeds radius ${maxR}`);
    }
  });
...
describe('voxelHollowBox', () => {
  it('generates a hollow box', () => {
    ...
    const interior = voxels.filter(v =>
      v.x > -2 && v.x < 2 && v.y > -2 && v.y < 2 && v.z > -2 && v.z < 2
    );
