...
import { createPostProcessState, updatePostProcessState, getActiveColorGrading, shouldEnableBloom } from './post-processing.js';
...
describe('updatePostProcessState', () => {
  it('interpolates bloomIntensity toward target', () => {
    const state = createPostProcessState();
    const config = { bloomIntensity: 1.0, bloomThreshold: 0.8, vignetteAmount: 0.7, colorGrade: 'desaturated' };
  ...
  it('interpolates vignetteAmount toward target', () => {
    const state = createPostProcessState();
    const config = { bloomIntensity: 0, bloomThreshold: 1.0, vignetteAmount: 0.9, colorGrade: 'warm' };
    const updated = updatePostProcessState(state, config, 1.0);
  ...
  it('converges to target over time', () => {
    ...
    const config = { bloomIntensity: 0.8, bloomThreshold: 0.8, vignetteAmount: 0.7, colorGrade: 'desaturated' };
    for (let i = 0; i < 200; i++) {
  ...
  it('dt of zero returns unchanged state', () => {
    const state = createPostProcessState();
    const config = { bloomIntensity: 1.0, bloomThreshold: 0.8, vignetteAmount: 0.9, colorGrade: 'desaturated' };
  ...
  it('clamps bloomIntensity to [0, 1]', () => {
    let state = createPostProcessState();
    const config = { bloomIntensity: 5.0, bloomThreshold: 0.8, vignetteAmount: 0.5, colorGrade: 'warm' };
    for (let i = 0; i < 300; i++) {
      state = updatePostProcessState(state, config, 0.1);
    }
...
describe('getActiveColorGrading', () => {
  it('returns warm tint values for warm color grade', () => {
    const state = { ...createPostProcessState(), colorGrade: 'warm', warmTint: 0.5, desaturationFactor: 0 };
  ...
  it('returns desaturation values for desaturated color grade', () => {
    const state = { ...createPostProcessState(), colorGrade: 'desaturated', warmTint: 0, desaturationFactor: 0.7 };
    const grading = getActiveColorGrading(state);
  ...
  it('warm tint lerps based on color grade blend', () => {
    const state = { ...createPostProcessState(), colorGrade: 'warm', warmTint: 0.8, desaturationFactor: 0.1 };
    const grading = getActiveColorGrading(state);
    assert.ok(grading.warmTint > 0.7);
  });

  it('desaturation factor is between 0 and 1', () => {
    const state = { ...createPostProcessState(), colorGrade: 'desaturated', warmTint: 0, desaturationFactor: 0.5 };
    const grading = getActiveColorGrading(state);
