...
describe("updateStamina with rest spots", () => {
  ...
  it("rest spot regen uses REST_REGEN_RATE constant", () => {
    ...
    assert.ok(
      Math.abs(next.current - expected) < 0.01,
      `expected ${expected}, got ${next.current}`,
    );
  });
...
describe("updateStamina", () => {
  it("drains when sprinting on ground", () => {
    ...
    const next = updateStamina(
      state,
      { isSprinting: true, isGrounded: true, isClimbing: false },
      0.1,
    );
    assert.ok(next.current < STAMINA_CONSTANTS.MAX);
  });
  ...
  it("uses climb drain rate when actively climbing", () => {
    ...
    const next = updateStamina(
      state,
      { isSprinting: false, isGrounded: false, isClimbing: true, isActivelyClimbing: true },
      1.0,
    );
  ...
  it("regens when grounded and not sprinting and not climbing", () => {
    ...
    const next = updateStamina(
      state,
      { isSprinting: false, isGrounded: true, isClimbing: false },
      0.1,
    );
    assert.ok(next.current > 50);
  });
