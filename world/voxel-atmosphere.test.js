...
describe('AO_CURVE', () => {
  ...
  it('has correct values', () => {
    assert.deepStrictEqual(AO_CURVE, [0.45, 0.65, 0.80, 1.0]);
  });
...
describe('updateFogForVoxels', () => {
  ...
  it('does not exceed reasonable fog density', () => {
    ...
    assert.ok(settings.sceneFogDensity < 0.05, `fog density ${settings.sceneFogDensity} should be < 0.05`);
...
describe('updateBloomForVoxels', () => {
  ...
  it('raises bloom threshold above default 0.1', () => {
    ...
    assert.ok(settings.bloomThreshold > 0.1, `threshold ${settings.bloomThreshold} should be > 0.1`);
  });

  it('threshold is at most 0.2', () => {
    const settings = updateBloomForVoxels({ bloomThreshold: 0.1 });
    assert.ok(settings.bloomThreshold <= 0.2, `threshold ${settings.bloomThreshold} should be <= 0.2`);
  });

  it('increases bloom intensity for emissive voxels', () => {
    const base = { bloomThreshold: 0.1, bloomIntensity: 0.5 };
    const settings = updateBloomForVoxels(base);
    assert.ok(settings.bloomIntensity > base.bloomIntensity, 'voxel bloom intensity should be higher');
  });
  ...
  it('bloom intensity stays in reasonable range', () => {
    ...
    assert.ok(settings.bloomIntensity <= 3.0, `bloom intensity ${settings.bloomIntensity} should be <= 3.0`);
  });
...
describe('createVoxelShadowConfig', () => {
  ...
  it('has shadow map size of at least 2048', () => {
    const config = createVoxelShadowConfig();
    assert.ok(config.shadowMapSize >= 2048, `shadow map size ${config.shadowMapSize} should be >= 2048`);
  });
  ...
  it('has shadow normal bias in reasonable range', () => {
    ...
    assert.ok(config.shadowNormalBias <= 0.1, `normal bias ${config.shadowNormalBias} should be <= 0.1`);
  });
