...
export function generateVoxelIsland({
  centerX,
  centerZ,
  radius,
  maxHeight,
  ...
  for (let wz = minZ; wz <= maxZ; wz++) {
    ...
    for (let wx = minX; wx <= maxX; wx++) {
      ...
      for (let wy = minY; wy <= maxY; wy++) {
        const dy = wy >= cY ? (wy - cY) / topH : (wy - cY) / botH;
  ...
  for (const k of solid) {
    ...
    if (!solid.has(wx + "," + (wy + 1) + "," + wz)) {
      ...
      hDist > 0.5 &&
      (!solid.has(wx + 1 + "," + wy + "," + wz) ||
        !solid.has(wx - 1 + "," + wy + "," + wz) ||
        !solid.has(wx + "," + wy + "," + (wz + 1)) ||
        !solid.has(wx + "," + wy + "," + (wz - 1)))
    ) {
      bt = BlockType.MOSS_STONE;
    }
...
export function generateVoxelSteppingStones({
  fromX,
  fromZ,
  toX,
  toZ,
...
export function generateVoxelRuins({
  centerX,
  centerZ,
  seed,
  voxelStorage,
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let attempt = 0; attempt < 50; attempt++) {
      ...
      for (const e of ruins) {
        if (Math.hypot(x - e.x, z - e.z) < minDist) {
          valid = false;
          break;
        }
      }
      ...
      if (valid) {
        ...
        if (isPillar) {
          for (let dy = 0; dy < height; dy++) {
            voxelStorage.setBlock(
              rx,
              baseY + dy,
              rz,
              dy === height - 1 ? BlockType.CRACKED_STONE : BlockType.STONE,
            );
          }
        } else {
