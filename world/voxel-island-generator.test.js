...
describe('generateVoxelIsland', () => {
  it('generates blocks within radius', () => {
    ...
    generateVoxelIsland({ centerX: 0, centerZ: 0, radius: 10, maxHeight: 4, seed: 42, voxelStorage: storage });
    let found = false;
  ...
  it('generates no blocks far outside radius', () => {
    ...
    for (let x = 15; x <= 20; x++) {
      for (let z = 15; z <= 20; z++) {
        assert.strictEqual(findTopBlock(storage, x, z), -1, `block at (${x},${z}) is outside radius`);
      }
    }
  });
  ...
  it('deep blocks are STONE', () => {
    const storage = createVoxelStorage();
    generateVoxelIsland({ centerX: 0, centerZ: 0, radius: 10, maxHeight: 6, seed: 42, voxelStorage: storage });
    const topY = findTopBlock(storage, 0, 0);
  ...
  it('hub island generates with correct radius', () => {
    const storage = createVoxelStorage();
    generateVoxelIsland({ centerX: 0, centerZ: 0, radius: 60, maxHeight: 8, seed: 42, voxelStorage: storage, type: 'hub' });
  ...
  it('arena island generates with correct radius', () => {
    const storage = createVoxelStorage();
    generateVoxelIsland({ centerX: 0, centerZ: 0, radius: 40, maxHeight: 5, seed: 100, voxelStorage: storage, type: 'arena' });
  ...
  it('is deterministic with same seed', () => {
    ...
    generateVoxelIsland({ centerX: 0, centerZ: 0, radius: 10, maxHeight: 4, seed: 42, voxelStorage: s1 });
    generateVoxelIsland({ centerX: 0, centerZ: 0, radius: 10, maxHeight: 4, seed: 42, voxelStorage: s2 });
    for (let x = -10; x <= 10; x += 3) {
      for (let z = -10; z <= 10; z += 3) {
        for (let y = 0; y <= 35; y++) {
          assert.strictEqual(s1.getBlock(x, y, z), s2.getBlock(x, y, z), `mismatch at (${x},${y},${z})`);
        }
      }
    }
  });
...
describe('generateVoxelSteppingStones', () => {
  it('generates blocks between two points', () => {
    const storage = createVoxelStorage();
    generateVoxelSteppingStones({ fromX: 0, fromZ: 0, toX: 50, toZ: 0, seed: 42, voxelStorage: storage, baseY: 20 });
    let found = false;
    for (let x = 10; x <= 40 && !found; x += 3) {
      for (let z = -4; z <= 4 && !found; z++) {
        for (let y = 18; y <= 22; y++) {
          if (storage.getBlock(x, y, z) !== BlockType.AIR) { found = true; break; }
        }
      }
    }
  ...
  it('uses STONE and MOSS_STONE', () => {
    ...
    outer: for (let x = 10; x <= 40; x++) {
      for (let y = 18; y <= 22; y++) {
        ...
        if (b === BlockType.STONE || b === BlockType.MOSS_STONE) { foundStone = true; break outer; }
      }
    }
...
describe('generateVoxelRuins', () => {
  it('returns correct count of ruins', () => {
    const storage = createVoxelStorage();
    const ruins = generateVoxelRuins({ centerX: 0, centerZ: 0, radius: 20, seed: 42, voxelStorage: storage, count: 5 });
    assert.strictEqual(ruins.length, 5);
  });

  it('ruins contain non-air blocks', () => {
    const storage = createVoxelStorage();
    const ruins = generateVoxelRuins({ centerX: 0, centerZ: 0, radius: 20, seed: 42, voxelStorage: storage, count: 3, baseY: 20 });
  ...
  it('ruins use CRACKED_STONE or STONE', () => {
    ...
    outer: for (const ruin of ruins) {
      for (let dy = 0; dy < 6; dy++) {
        ...
        if (b === BlockType.CRACKED_STONE || b === BlockType.STONE) { found = true; break outer; }
      }
    }
